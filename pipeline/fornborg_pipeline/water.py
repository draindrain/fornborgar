...
#: Added to `provenance.processing` when the bundle ships the §12 delta form.
DELTA_PROCESSING_STEP = "sea-connectivity shipped as int16 delta vs. the DEM (contract §12)"
...
def run_connectivity(cfg: SiteConfig, dem_dm, transform, delta: bool = True) -> str:
    """Build and write the §7 connectivity grid, then report what it excludes.

    Returns the asset key the manifest should carry: `"waterConnectDelta"` for the
    §12 shipping form (the default — ~10x smaller), `"waterConnect"` for the
    absolute grid the pre-v1.5 bundles carry.
    """
    started = time.time()
    ...
    if delta:
        # §12: `connect - dem` is zero in the 90-odd % of cells outside a
        # depression, so the same information deflates ~10x smaller. Remove any
        # absolute grid an earlier build left behind — the manifest stops
        # pointing at it, and a stale file would only bloat the upload.
        delta_dm = connect_delta(connect_dm, dem_dm)
        path = write_connect_delta_grid(
            cfg.out_dir / WATER_CONNECT_DELTA_PATH,
            delta_dm,
            transform,
            cfg,
            cfg.context,
        ...
        stale = cfg.out_dir / WATER_CONNECT_PATH
        if stale.exists():
            stale.unlink()
            print(f"  removed the superseded absolute grid {stale.name}")
        asset_key = "waterConnectDelta"
    else:
        path = write_connect_grid(cfg.out_dir / WATER_CONNECT_PATH, connect_dm, transform, cfg)
        asset_key = "waterConnect"
    print(f"  wrote {path} ({path.stat().st_size / 1e6:.2f} MB)")
    _check_identical_profile(cfg.out_dir / cfg.context.path, path)
    return asset_key


def run_ring_connectivity(
    cfg: SiteConfig, manifest: dict, delta: bool = True
) -> tuple[str, str, str] | None:
    """Build the §11 far-water connect grid on the 16 km ring, when the site has one.

    Returns `(ring_dem_path, asset_key, asset_path)` for the manifest patch, or
    None when the site ships no rings (feature off, never an error). The grid
    exists only for far-water *rendering* — analysis stays on the context connect
    grid (§7). As for the context grid, the §12 delta is the default shipping form.
    """
    ...
    if delta:
        delta_name = ring_connect_delta_path(ring_spec)
        path = write_connect_delta_grid(
            cfg.out_dir / delta_name,
            connect_delta(connect_raw, dem_raw),
            transform,
            cfg,
            ring_spec,
            delta_name,
        )
        stale = cfg.out_dir / connect_spec.path
        if stale.exists():
            stale.unlink()
            print(f"  removed the superseded absolute grid {stale.name}")
        asset_key, asset_path = "waterConnectDelta", delta_name
    else:
        path = write_ring_connect_grid(
            cfg.out_dir / connect_spec.path, connect_raw, transform, cfg, ring_spec
        )
        asset_key, asset_path = "waterConnect", connect_spec.path
    print(f"  wrote {path} ({path.stat().st_size / 1e6:.2f} MB)")
    return ring_spec.path, asset_key, asset_path
...
def patch_manifest(cfg: SiteConfig, water_meta: dict, asset_key: str = "waterConnectDelta") -> dict:
    """Add the water pair to the site's committed manifest and re-validate it."""
    manifest_path = cfg.out_dir / "manifest.json"
    manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
    is_delta = asset_key == "waterConnectDelta"
    add_water_assets(
        manifest,
        SHORELINE_PATH,
        connect_path=None if is_delta else WATER_CONNECT_PATH,
        delta_path=WATER_CONNECT_DELTA_PATH if is_delta else None,
        source={
        ...
        processing=(*PROCESSING_STEPS, *((DELTA_PROCESSING_STEP,) if is_delta else ())),
    )
...
def run(site_id: str, force_download: bool = False, delta: bool = True) -> dict:
    """Derive both water assets for one site and wire them into the manifest.

    `delta=True` (the default) ships the §12 delta encoding; `delta=False` writes
    the pre-v1.5 absolute grids, which is what regenerating an old bundle in place
    needs.
    """
    ...
    asset_key = run_connectivity(cfg, dem_dm, transform, delta=delta)

    print("-- far-water connectivity (contract §11)")
    committed = json.loads((cfg.out_dir / "manifest.json").read_text(encoding="utf-8"))
    ring_connect = run_ring_connectivity(cfg, committed, delta=delta)

    print("-- manifest")
    manifest = patch_manifest(cfg, water_meta, asset_key=asset_key)
    if ring_connect is not None:
        ring_path, ring_key, ring_asset = ring_connect
        set_ring_water_connect(
            manifest,
            ring_path,
            **{"delta_path" if ring_key == "waterConnectDelta" else "connect_path": ring_asset},
        )
...
@click.option(
    "--absolute-connect",
    is_flag=True,
    help="Write the pre-v1.5 absolute connectivity grids instead of the §12 deltas.",
)
def cli(site_id: str, force_download: bool, absolute_connect: bool) -> None:
    """Derive shoreline.json + the connectivity grid and patch the site manifest."""
    try:
        run(site_id, force_download=force_download, delta=not absolute_connect)
